<template>
    ...
    <li v-for="(menuItem, index) in menuFiltre" :key="index" class="sidebar-list" 
...
// const menuFiltre = computed(() => {
//     return menu.filter(menuItem => checkPermission(menuItem.role));
// });
console.log(menu,"menuItemmenuItem");

const menuFiltre = computed(() => {
    return menu
    .map(menuItem => {
        // Filtre les sous-éléments dans `children` si l'élément en contient
        const filteredChildren = menuItem.children
        ? menuItem.children.filter(child => checkPermission(child.role))
        : [];

        // Filtre l'élément principal si l'utilisateur a la permission ou s'il contient des sous-éléments autorisés
        if (checkPermission(menuItem.role) || filteredChildren.length > 0) {
        return {
            ...menuItem,
            children: filteredChildren
        };
        }
        return null; // Si l'élément principal et les sous-éléments ne sont pas autorisés
    })
    .filter(Boolean); // Supprime les éléments null
});


console.log(menuFiltre,"menuFiltremenuFiltre");
