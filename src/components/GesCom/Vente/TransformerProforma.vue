<template>
  <div class="card mb-25 border-0 rounded-0 bg-white add-user-card">
    <div class="card-body p-15 p-sm-20 p-md-25 p-lg-30 letter-spacing">
      <Form @submit="transformerProforma" :validation-schema="venteSchema" :keep-values="valuess">
        <div class="row gx-1">
          <div class="col">
          ...
          <div class="col-md-2">
            <div class="form-group mb-2 mb-sm-2 mb-md-2">
              ...
              <Field name="typeFacture" v-model="typeFacture" v-slot="{ field }">
                <Multiselect :options="['Facture de vente']" 
                  :searchable="false" 
                  track-by="label" 
                  label="label" 
                  v-model="field.value" 
  ...
  <div class="form-group mb-2 mb-sm-2 mb-md-2">
    ...
    <!-- ✅ input-group pour coller le bouton au champ -->
    <div class="input-group">
      <Field name="montantReçu" type="text" v-model.number="montantReçu"
        class="form-control shadow-none fs-md-15 text-black" 
        placeholder="Montant reçu" readonly />
      <button type="button" class="btn btn-primary" 
        data-bs-toggle="modal" data-bs-target="#billetagePaiementModal"
        title="Ouvrir le billetage">
    ...
    <small v-if="reliquatNonRendu > 0" class="text-warning d-block mt-1">
      <i class="fa fa-exclamation-triangle"></i> Reliquat non rendu: {{ reliquatNonRendu }} Fcfa
            ...
            <div class="accordion-body">
              ...
              <div class="tab-content" id="myTabContent">
                          ...
                          <div>
                              ...
                              <div class="col">
                                <label class="d-block text-black fw-semibold">
                                  ...
                                  <i class="fa fa-info-circle text-muted ms-1" 
                                     v-if="produit.prixMin && produit.prixMax"
                            ...
                            <div class="row gx-1">
                              ...
                              <div class="col">
                                ...
                                <input type="text" v-model.number="produit.valeurRemise" class="form-control" placeholder=""
                                  @keyup="eventProduit" />
                              </div>
                              <div class="col">
                                <label class="d-block text-black fw-semibold">
                ...
                <div class="tab-pane fade p-10" id="home2-tab-pane" role="tabpanel" tabindex="0">
                 ...
                 <div v-if="prixErrorMessage" 
              class="alert alert-danger mx-auto mb-2 py-1" 
              role="alert" 
          ...
          <div class="modal fade" id="stockDetailsModal" tabindex="-1" aria-labelledby="stockDetailsModalLabel"
           ...
           <!-- Modal Billetage -->
          <div class="modal fade" id="billetagePaiementModal" tabindex="-1" aria-labelledby="billetagePaiementModalLabel"
            aria-hidden="true">
              ...
              <div class="modal-content">
                ...
                <div class="modal-body">
                            ...
                            <div class="row align-items-center mb-2">
                              ...
                              <div class="col-md-4 px-1">
                                <input type="number" min="0" v-model.number="billetage.qteBillet"
                                  ...
                                  @keypress="restrictInput($event)" class="form-control form-control-sm" placeholder="0" />
                              </div>
                              <div class="col-md-4 px-1">
                                <input type="text" v-model="billetage.montant" readonly class="form-control form-control-sm" />
                              </div>
                            </div>
                          </template>
                        </div>
                      ...
                      <div class="border border-danger">
                        ...
                        <div class="px-3 py-2">
                          ...
                          <div v-if="montantTotalSortie > reliquatCalcule" class="alert alert-danger mt-2 py-2">
                            <small>Le billetage de sortie ({{ montantTotalSortie }} Fcfa) dépasse le reliquat ({{ reliquatCalcule }} Fcfa).</small>
                          </div>
                        </div>
                      </div>
                    </div>
                  ...
                  <div v-if="reliquatNonRendu > 0" class="alert alert-warning mt-2">
                    <i class="fa fa-exclamation-triangle me-2"></i>Reliquat non rendu au client: {{ reliquatNonRendu }} Fcfa
                  </div>
                </div>
              ...
              <div class="col-md-4">
                <div class="btn-group" role="group">
                  ...
                  <button type="submit" class="btn btn-primary px-4" 
                    :disabled="isDisable || (showC && (!montantTotalEncaissement || montantTotalEncaissement < totalttc))"
                    :class="{ 'cursor-not-allowed': isDisable || (showC && (!montantTotalEncaissement || montantTotalEncaissement < totalttc)) }"
>
                    <i class="fa fa-exchange me-2"></i> Transformer en vente
                  </button>
                  ...
                  <ul class="dropdown-menu">
                    <li>
                      <a class="dropdown-item" href="#" @click.prevent="$event => $parent.$emit('submit')"
                         :class="{ 'disabled': showC && (!montantTotalEncaissement || montantTotalEncaissement < totalttc) }"

                        >
                        <i class="fa fa-exchange"></i> Transformer en vente
                      </a>
                    </li>
                    <li><a class="dropdown-item" href="#" @click.prevent="handleSaveAndPrint" :class="{ 'disabled': isGeneratingPdf || (showC && (!montantTotalEncaissement || montantTotalEncaissement < totalttc)) }">
                      <i v-if="!isGeneratingPdf" class="fa fa-print"></i>
                      <i v-else class="fa fa-spinner fa-spin"></i>
                      {{ isGeneratingPdf ? 'Génération...' : 'Transformer et imprimer' }}
                    </a></li>
              ...
              <div class="col-md-3">
                <div class="form-group mb-0">
                  <label class="d-block text-black fw-semibold mb-1">
                    Format impression <span class="text-danger">*</span>
                  </label>
                  <Field name="formatImpression" v-model="formatImpression" v-slot="{ field }">
                    <Multiselect :options="['A4', 'A5', 'A6', 'T1', 'T2']" :searchable="true" v-model="field.value"
                      v-bind="field" placeholder="Sélectionner" />
                  </Field>
            ...
            </div> <!-- fin de .row align-items-end -->
          </div> <!-- fin de .col-md-12 mt-5 -->
...
export default defineComponent({
  ...
  setup: () => {
    ...
    const proformaId = ref(route.params.id);
    ...
    const validerBilletage = () => {
      billetageError.value = '';
    
      if (!montantTotalEncaissement.value || montantTotalEncaissement.value < totalttc.value) {
        billetageError.value = `Montant insuffisant (${totalttc.value - montantTotalEncaissement.value || 0} Fcfa manquant)`;
        return;
      }
    
      if (montantTotalSortie.value > reliquatCalcule.value) {
        billetageError.value = `Sortie trop importante (${montantTotalSortie.value - reliquatCalcule.value} Fcfa en trop)`;
        return;
      }
    ...
    const selectProd = async (selectedProduit, produit) => {
      ...
      produit.prixHt = Number(dataprice.data.prixHT);
       produit.prixMin = Number(dataprice.data.prixMin || dataprice.data.prixHT); 
      produit.prixMax = Number(dataprice.data.prixMax || dataprice.data.prixTTC); 
    ...
    const chargerProforma = async () => {
  try {
    const response = await ApiService.get(`/commandeClients/${proformaId.value}`);
    console.log("infooooooooooooooooo", response);
    
    if (response.data.code === 200) {
      const proforma = response.data.data;
      console.log("valllkkkk", proforma);

      if (proforma.client) {
        const clientLabel = proforma.client.nomClient 
          ? `${proforma.client.nomClient} ${proforma.client.prenomClient}` 
          : proforma.client.raisonSociale;
        
        if (!clientOptions.value.find(c => c.value === proforma.client.id)) {
          clientOptions.value.push({
            value: proforma.client.id,
            label: clientLabel
          });
        }
        
       clientCharger.value = proforma.client.id;
      }
      
      selectedClient.value = false; 

      if (venteForm.value) {
        venteForm.value.setFieldValue('client', proforma.client?.id);
        venteForm.value.setFieldValue('dateCommande', proforma.dateCommande);
        venteForm.value.setFieldValue('typeFacture', 'Facture de vente');
        venteForm.value.setFieldValue('tauxAIB', proforma.tauxAib);
        venteForm.value.setFieldValue('formatImpression', proforma.formatImpression || 'A4');
      }

     
      const factureProforma = proforma.commandeFactures?.[0];
      const produitsProforma = factureProforma?.factureprocondis || [];

      console.log("factureProforma", factureProforma);
      console.log("produitsProforma", produitsProforma);

      if (produitsProforma.length > 0) {
        produits.value = produitsProforma.map((ligne) => ({
          commande_clit_id: ligne.id,
          magasin: null, 
          produit: `${ligne.produitcondi?.id}|${ligne.produitcondi?.produit?.nomProd} [ ${ligne.produitcondi?.conditionnement?.libelle || ''} ]`,
          qtite: ligne.qtite,
          prixTtc: ligne.prixTtc || 0,
          prixHt: ligne.prixHt || (ligne.prixTtc / 1.18) || 0,
          prixMin: 0,
          prixMax: 0,
          taxeSpecifique: 0,
          typeRemise: '',
          ...
          montantTtc: ligne.montantTtc || (ligne.prixTtc * ligne.qtite) || 0,
          montantTs: 0,
          montantHtApresRemise: ligne.montantHtApresRemise || ligne.montantHt || 0,
          libelleProduit: ligne.produitcondi?.produit?.nomProd || '',
          groupeTaxeProduit: "",
          carton: 0,
          piece: 0,
          stock: 0,
        ...
        console.warn("Aucun produit trouvé dans factureprocondis");
        error("Aucun produit trouvé dans cette facture proforma");
      }

      if (proforma.elements && proforma.elements.length > 0) {
        elements.splice(0, elements.length);
        proforma.elements.forEach((elem) => {
          elements.push({
            titre: elem.titre,
            description: elem.description,
            taillePolice: elem.taillePolice,
            alignement: elem.alignement
          });
        });
      }
      ...
      success("Données de la facture proforma chargées avec succès");
    }
  } catch (err: any) {
    console.error('Erreur complète:', err);
    console.error('Réponse serveur:', err.response?.data);
    error(err.response?.data?.message || "Erreur lors du chargement de la facture proforma");
  ...
  if (proformaId.value) {
    await chargerProforma();
  }
});

    const transformerProforma = async (values, { resetForm }, shouldNormalize = false) => {
  if (!values.client) {
    error("Veuillez sélectionner un client");
    return null;
  }

  // Pour la transformation, on force le type de commande
  if (!values.typeCommande) {
    error("Veuillez sélectionner le type de commande");
    return null;
  }

 if (typeC.value === "Au comptoir" && (!montantTotalEncaissement.value || montantTotalEncaissement.value < totalttc.value)) {
  ...
  const produitsWithoutMagasin = produits.value.filter(p => !p.magasin);
  if (produitsWithoutMagasin.length > 0) {
    error("Veuillez sélectionner un magasin pour tous les produits");
    return null;
  }
  
  const payload = {
    typeCommande: values.typeCommande,
    montantReçu: values.montantReçu || 0,
    normaliser: shouldNormalize,
    magasins: produits.value.map((produit) => ({
      produitCondiId: parseInt(produit.produit.split("|")[0]),
      magasinId: typeof produit.magasin === 'object' ? (produit.magasin as any).value : produit.magasin
    })),

        billetage: typeC.value === "Au comptoir" ? {
        encaissement: billetageList,
        sortie: billetageSortieList,
  ...
  console.log("Payload transformation proforma:", payload);
  
  try {
    const response = await ApiService.put(`/commandeClients/${proformaId.value}/transformer-proforma`, payload);

    if (response.data.code === 200) {
      success(response.data.message || "Facture proforma transformée en vente avec succès");
      resetForm();
      resetFormData(); 
    ...
    const errorMsg = response.data.message || "Erreur lors de la transformation";
    error(errorMsg);
    return null;
  ...
  try {
    ...
    const formData = {
      ...
      typeFacture: typeFacture.value,
      tauxAIB: valeurAib.value,
      montantReçu: montantReçu.value || 0,
      client: clientCharger.value,
      formatImpression: formatImpression.value,
    };

    
    const response = await transformerProforma(formData, { resetForm: () => {} }, true);
 ...
 watch(typeFacture, (newValue) => {
  if (newValue === 'Facture proforma') {
    ...
    // Sauvegarder la vente en cours dans sessionStorage
    ...
    async function addRowProduit() {
      ...
      // const reliquatCalculer = () => {
      // if (!montantReçu.value || montantReçu.value === '' || montantReçu.value === '0' || totalttc.value === 0) {
      //   reliquat.value = 0;
      //   return 0;
      // }
  
//   const montantStr = String(montantReçu.value).replace(/\s/g, '');
//   const montant = parseFloat(montantStr);
//     const ttc = totalttc.value || 0;  
...
//   reliquat.value =  parseFloat((montant - ttc).toFixed(2));
//    console.log('Type de reliquat:', typeof reliquat.value);
//   console.log('Valeur de reliquat:', reliquat.value);
};

watch(typeC, (newValue) => {
...
// watch(montantReçu, (newMontantReçu) => {
  
//     if(newMontantReçu && newMontantReçu != 0) {
//       reliquatCalculer();
...
// watch(totalttc , (newMontantTtc) => {
//  if(montantReçu.value && montantReçu.value != 0) {
//     reliquatCalculer();
//   }
...
// });
  ...
  // Convertir les valeurs en nombres pour éviter la concaténation
