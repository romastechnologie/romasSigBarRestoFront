<template>
  ...
  <div class="col-md-3">
              <div class="form-group ">
                  ...
                  <Multiselect :searchable="true" :options="lestypes"  v-model="_type"
                    placeholder="Choisir" />
              </div>
          </div>
...
export default defineComponent({
  ...
  setup(){
    ...
    onMounted(async () => {
      ...
      getAllTransaction(point_vente.value);
    });
    ...
    const handlePaginate = ({ page_, limit_ }) => {
      try {
        page.value = page_;
        getAllTransaction(page_, limit_);
      } catch (error) {
        //
      }
    };
 ...
 watch(point_vente, () => {
    ...
    watch(_type, () => {
     ...
     function rechercher(){
      getAllTransaction(point_vente.value,page.value, limit.value, searchTerm.value,_type.value );
    }

    // END PAGINATE

    function getAllTransaction(point_vent,page = 1, limi = 10, searchTerm = '',type="") {
      return ApiService.get(`/all/transactions/?page=${page}&limit=${limi}&mot=${searchTerm}}&type=${type}&pointvente=${point_vent}&`)
        .then(({ data }) => {
