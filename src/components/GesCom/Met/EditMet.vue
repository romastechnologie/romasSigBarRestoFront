<template>
  <div class="card mb-25 border-0 rounded-0 bg-white add-user-card">
    <div class="card-header">
      <h3 class="text-black fw-semibold">Modifier un met</h3>
    </div>
    <div class="card-body p-15 p-sm-20 p-md-25 p-lg-30 letter-spacing">
      <Form ref="metForm" @submit="editMet" :validation-schema="metSchema" :initial-values="metForm">
        <div class="row gx-1">
          <div class="col-md-12">
                ...
                <div class="row gx-1">
                  <div class="col-md-3">
                    <div class="form-group mb-3 mb-sm-3 mb-md-3">
                      ...
                      <Field type="text" v-model="codeMet" name="code"
          ...
          <div class="col-md-12">
            <div class="d-flex align-items-center">
              ...
              <router-link to="/mets/liste-mets" class="btn btn-danger"><i
...
export default defineComponent({
  ...
  setup: () => {
    const metSchema = Yup.object().shape({
      ...
      categorieMet: Yup.string().required("La catégorie de met est obligatoire"),
      ...
      ApiService.get("/mets/" + id.toString())
        .then(({ data }) => {
          ...
          for (const key in data.data) {
            metForm.value?.setFieldValue(key, (typeof data.data[key] === 'object' && data.data[key] !== null) ? data.data[key].id : data.data[key]);
          }
          ...
          codeMet.value = data.data["codeMet"];
    ...
    const editMet = async (values, { resetForm }) => {
      const data = metForm.value;
      ...
      ApiService.put("/mets/" + route.params.id, values)
        .then(({ data }) => {
...
const fetchCategorieMet = async () => {
      try {
        ...
        const categorieMetData = response.data.data.data;
        console.log(categorieMetData, "categorieMetData");
        categorieOptions.value = categorieMetData.map((categorieMet) => ({
          value: categorieMet.id,
          label: `${categorieMet.code} - ${categorieMet.libelle} ${categorieMet.categorieMet ? '(' + categorieMet.categorieMet.libelle + ')' : ""}`,
        }));
      } catch (error) {
        //
      }
    ...
    const fetchConditionnements = async () => {
      try {
        ...
        metsss.value = conditionnementsData;
    ...
    const fetchConditionnementugs = async () => {
      try {
        ...
        metsss.value = conditionnementgData;
    ...
    const pays = [
     ...
     const isPhotoPiece = (typeId: string | number) => {
      return String(typeId) === '5' || String(typeId) === '9';
    };
