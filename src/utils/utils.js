...
const getUrlApiForProductFiles = (nomFichier = null) => {
...
//   });
// } 
const optionPointVente = () => {
    const pointVentes = JwtService.getPointDeVentes();
    console.log("Données reçues:", pointVentes);
    return pointVentes ? pointVentes.map((point) => ({
        value: point.id,
...
const supRowElement = (id, element, route, entite) => {
    Swal.fire({
        text: "Vous êtes sur le point de supprimer " + entite + ". Etes-vous sûr ?",
        icon: "warning",
        buttonsStyling: true,
...
export { optionPointVente, getCurrentDateTime, getDatePlusXDays, generateUuid, getUrlApiForProductFiles, ajouterPeriode, onFileChange, removeModalBackdrop, suppression, separateur, hideModal, getAssetPath, format_Date, showModal, format_date, success, error, getUrlApiForFiles, };
