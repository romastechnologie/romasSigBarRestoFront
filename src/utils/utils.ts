...
const optionPointVente = ():[] => {
  ...
  return pointVentes ? pointVentes.map((point: any) => ({
    value: point.id,
...
const supRowElement = (id: number, element: any, route: string, entite: string) => {
  Swal.fire({
    text: "Vous êtes sur le point de supprimer " + entite + ". Etes-vous sûr ?",
    icon: "warning",
    buttonsStyling: true,
...
export {
  optionPointVente,
  getCurrentDateTime,getDatePlusXDays,generateUuid,getUrlApiForProductFiles, ajouterPeriode, onFileChange, removeModalBackdrop, suppression, separateur, hideModal, getAssetPath, format_Date, showModal, format_date, success, error,warning,monSwal,getUrlApiForFiles,calculerDuree,
};
